        ...
        if isinstance(item, FileItem):
            ...
            if etag:
                ...
                if key:
                    ...
                    key = Key(self.bucket, name=key_str)
                    etag = ""
    ...
    def process_item(self, item, spider):
        if isinstance(item, FileItem):
            ...
            if etag:
                ...
                if key:
                    ...
                    headers = {
                        ...
                        "x-archive-meta-date": datetime.strptime(
            ...
            with NamedTemporaryFile(delete=True) as fd:
                ...
                if r.status_code == 304:
                    logger.info(
                        "%s already exists at archive.org and is up to date", key_str
                    )
                elif r.status_code == 200:
                    for chunk in r.iter_content(chunk_size=None):
                        fd.write(chunk)
                    logger.info(
                        "Uploading %s to archive.org identifier %s",
                        item["path"],
                        identifier,
                    )
                    if not key:
